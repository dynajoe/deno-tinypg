...
export type HookCollection = {
  [P in keyof Required<TinyHooks>]: TinyHooks[P][];
};
...
export interface TinyHookLifecycle {
  preSql?: (
    tiny_ctx: TinyCallContext,
    params: [string, TinyPgParams],
  ) => HookResult<[string, TinyPgParams]>;
  preRawQuery?: (
    tiny_ctx: TinyCallContext,
    params: [string, TinyPgParams],
  ) => HookResult<[string, TinyPgParams]>;
...
export type TinyPgParams = {
  [key: string]: null | undefined | TinyPgPrimitive | TinyPgPrimitive[];
};
